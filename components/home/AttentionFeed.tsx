...
const DEFAULT_ITEMS: FeedItem[] = [
  {
    ...
    description: "Your calm builder companion. Set up your integrations to get started.",
    type: "info",
  ...
  },
  {
    ...
    description: "Connect your GitHub account to track repositories and deployments.",
    type: "info",
...
function typeConfig(type: FeedItem["type"]) {
  switch (type) {
    case "error":
      return { color: "bg-red-400", icon: AlertCircle, badgeClass: "bg-red-500/20 text-red-400 border-red-500/30" };
    case "warning":
      return { color: "bg-yellow-400", icon: AlertTriangle, badgeClass: "bg-yellow-500/20 text-yellow-400 border-yellow-500/30" };
    case "info":
    default:
      return { color: "bg-blue-400", icon: Info, badgeClass: "bg-blue-500/20 text-blue-400 border-blue-500/30" };
  }
}
    ...
    <Card className="relative overflow-hidden">
      <CardHeader className="pb-3">
        ...
        {unreadItems.length > 0 && (
          <Badge variant="secondary" className="w-fit text-xs bg-accent/20 text-accent border-accent/30">
        ...
        <AnimatePresence mode="popLayout">
          {sortedItems.length === 0 ? (
            ...
            >
              ...
              <p className="text-text-muted text-sm">All clear! Nothing needs attention.</p>
            </motion.div>
          ) : (
            <div className="space-y-1">
              {sortedItems.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    <div className={cn("h-2 w-2 rounded-full mt-2 shrink-0", config.color)} />
